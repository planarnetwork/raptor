...
Patterns for a whole feed are generated by `npm run patterns <gtfs.zip> <date>`, which builds the
timetable once and shares it with a pool of workers. Set `WORKERS` to size the pool; it defaults to
two fewer than the machine has cores.

### Transfer patterns for a whole feed

`npm run patterns <gtfs.zip> <date> [out.br]` plans every station in the feed and writes the
patterns to a single file. Set `WORKERS` to size the pool; it defaults to two fewer than the
machine has cores, and they share one timetable rather than a feed each.
...
stations it takes from the line above and what follows it. That is the same tree written out depth
...
The file is brotli compressed. A national feed comes to about 33MB for 34 million patterns.

```js
const {readPatterns} = require("raptor-journey-planner");

const lines = readline.createInterface({
  input: fs.createReadStream("patterns.br").pipe(zlib.createBrotliDecompress())
});
